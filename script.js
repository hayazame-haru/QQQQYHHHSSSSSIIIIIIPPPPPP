...
for(let i=1;i<=8;i++){
  defaultFiles[`pdf${i}`] = {
    ...
    path:(i===1
      ? "https://www.dropbox.com/scl/fi/pqk33f39lt5qj7xeiiy0a/pdf1.pdf?rlkey=3zi9u46pb1jtty96wi6q94hgl&st=6t1uf0ay&raw=1"
      : (i===8
          ? "https://www.dropbox.com/scl/fi/q1u9lqdjt8ee1dnary2ru/pdf8.pdf?rlkey=7j1ei1t8vpb1ohor87m6o0qmg&st=7vkngy1e&raw=1"
          : `pdf/pdf${i}.pdf`)),
    enabled: (i===1 || i===8) 
  };
}
...
function createWindow(title, type, data){
  ...
  if(type === "pdf"){
    ...
    iframe.src = data + "#toolbar=0&navpanes=0&scrollbar=0";
...
function dragElement(el, bar, taskEntry){
  ...
  function move(e){
    ...
    el.style.left = Math.min(Math.max(newLeft, 0), vw - el.offsetWidth) + "px";
    el.style.top  = Math.min(Math.max(newTop, 0), vh - el.offsetHeight) + "px";
  }
